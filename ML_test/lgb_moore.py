...
def createlogger(clfname):
  logger = logging.getLogger(clfname)
...
af_file = os.path.join(data_dir, af_filename)
after_df = pd.read_csv(af_file, names = names)
after_data = after_df[data_index].values
after_label = after_df[248].values
...
if __name__ == '__main__':
    ...
    lgb = LGBMClassifier(n_estimators=130)
    xgb = XGBClassifier(n_estimators=130)
    svm = SVC(max_iter=2000, kernel='linear')
    rbfsvm = SVC(max_iter=2000, kernel='rbf')
